...
Use this exact markdown link form (not a bare `@username`) — it pings the user and works even when they have no `@username` set, since it links by their numeric id instead. Write it as the first thing in `message`, followed by your reply, e.g. `Hello [{{USER_NAME}}](tg://user?id={{USER_ID}})! sure, here's...`. Where it's actually relevant to what they asked, also reference their known profile details (level, $ balance, rank) — but don't recite the whole profile line verbatim in every reply, work the relevant piece in naturally.
